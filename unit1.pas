...
var
  ...
  matrizProjecao, matrizRotacao:T4Array;
  ativarRotacao:Boolean;
  xr,yr,zr:integer;
...
begin
     ...
     aplicarProjecao:=false;
     inicializarZBuffer();
     perspectivaCavaleira(matrizProjecao);
     //desenharCuboParaleloEixos(200,200,200,100,clRed);
     //desenharCone(300,300,-30, 40,clBlue);
     desenharParaboloide(200,200,0,50,clRed);
     desenharCilindro(200,250,0,50,120,clGray);
     Form3.lambert:=false;
     Form4.phong:=False;

end;

//matriz que rotaciona em todos os eixos
procedure TForm1.inicializarMatrizRotacao();
begin
     matrizXY[1,1] := 1;
     matrizXY[1,2] := 0;
     matrizXY[1,3] := 0;
...
begin
    if ((x > 0) and (x <= 784) and (y > 0) and (y <= 562)) then
       ...
       begin
            ...
            if (ativarRotacao = true) then
            begin
                 multiplicacao(ponto, matrizRotacao, ponto);
            end;
